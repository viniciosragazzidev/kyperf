...
function Command({
  ...
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
        "flex size-full flex-col overflow-hidden rounded-xl bg-background p-1 text-popover-foreground",
        className
      )}
      {...props}
    />
...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      >
        <Command className="**:[[cmdk-root]]:h-auto **:[[cmdk-root]]:max-h-[min(80vh,480px)]">
          {children}
        </Command>
      </DialogContent>
    </Dialog>
    ...
    <div data-slot="command-input-wrapper" className="p-1 pb-0">
      <InputGroup className="h-8 bg-input/20 dark:bg-input/30">
        ...
        <CommandPrimitive.Input
          ...
          className={cn(
            "w-full h-8 text-xs/relaxed outline-hidden placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50",
            className
          )}
          {...props}
        />
